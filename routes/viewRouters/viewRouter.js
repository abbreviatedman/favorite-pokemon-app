...
router.get("/create-pokemon-form", async function(req, res) {
  try {
...
router.post('/log-in-user', async (req, res) => {
  try {
    ...
    if (!user) {
      res.render('error', {errorMessage: 'Username incorrect.'});
    ...
    console.log(error);
    res.render('error', {errorMessage: 'Sorry, that should not have happened! Some server error occurred.'});
  }
})
...
router.get('/user', async (req, res) => {
  try {
    ...
    res.render('user', {favoritePokemon: faves, username: user.username})
  } catch (error) {
    res.render('error', {errorMessage: 'Sorry, that should not have happened!'})
  }
})
...
router.get('/logout', async function(req, res) {
  try {
    ...
    res.render('error', {errorMessage: 'How did you possibly get here?!'})
  }
})
...
router.patch('/addFavorite/:pokemonId', async (req, res) => {
  try {
    ...
    res.render('error', {errorMessage: 'Something mysteeeerious happened here. (An error. An error happened.'})
  }
})
